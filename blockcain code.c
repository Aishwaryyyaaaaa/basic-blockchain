#include <stdio.h>
#include <string.h>
#include <stdint.h>

// SHA-256 constants
const uint32_t K[64] = {
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0b5f0, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
};

// Rotating a 32-bit value to the right
#define ROTR(x, n) ((x >> n) | (x << (32 - n)))

// Compression function for SHA-256
void sha256_transform(uint32_t state[8], const uint8_t data[64]) {
    uint32_t a, b, c, d, e, f, g, h, t1, t2, w[64];

    // Prepare the message schedule
    for (int i = 0; i < 16; i++) {
        w[i] = (data[i * 4] << 24) | (data[i * 4 + 1] << 16) | (data[i * 4 + 2] << 8) | data[i * 4 + 3];
    }
    for (int i = 16; i < 64; i++) {
        w[i] = w[i - 16] + (ROTR(w[i - 15], 7) ^ ROTR(w[i - 15], 18) ^ (w[i - 15] >> 3)) + w[i - 7] + (ROTR(w[i - 2], 17) ^ ROTR(w[i - 2], 19) ^ (w[i - 2] >> 10));
    }

    // Initialize working variables with current hash state
    a = state[0];
    b = state[1];
    c = state[2];
    d = state[3];
    e = state[4];
    f = state[5];
    g = state[6];
    h = state[7];

    // Main loop
    for (int i = 0; i < 64; i++) {
        t1 = h + (ROTR(e, 6) ^ ROTR(e, 11) ^ ROTR(e, 25)) + ((e & f) ^ (~e & g)) + K[i] + w[i];
        t2 = (ROTR(a, 2) ^ ROTR(a, 13) ^ ROTR(a, 22)) + ((a & b) ^ (a & c) ^ (b & c));
        h = g;
        g = f;
        f = e;
        e = d + t1;
        d = c;
        c = b;
        b = a;
        a = t1 + t2;
    }

    // Add the result to the current hash state
    state[0] += a;
    state[1] += b;
    state[2] += c;
    state[3] += d;
    state[4] += e;
    state[5] += f;
    state[6] += g;
    state[7] += h;
}

// Initialize the SHA-256 hash state
void sha256_init(uint32_t state[8]) {
    state[0] = 0x6a09e667;
    state[1] = 0xbb67ae85;
    state[2] = 0x3c6ef372;
    state[3] = 0xa54ff53a;
    state[4] = 0x510e527f;
    state[5] = 0x9b05688c;
    state[6] = 0x1f83d9ab;
    state[7] = 0x5be0cd19;
}

// Perform SHA-256 on the input message
void sha256_hash(const uint8_t *msg, size_t len, uint8_t *out) {
    uint32_t state[8];
    sha256_init(state);

    uint8_t buffer[64];
    size_t remaining = len;
    size_t offset = 0;

    while (remaining >= 64) {
        memcpy(buffer, msg + offset, 64);
        sha256_transform(state, buffer);
        offset += 64;
        remaining -= 64;
    }

    // Handle the remaining part of the message
    memset(buffer, 0, 64);
    memcpy(buffer, msg + offset, remaining);
    buffer[remaining] = 0x80;
    if (remaining >= 56) {
        sha256_transform(state, buffer);
        memset(buffer, 0, 64);
    }
    uint64_t bitlen = len * 8;
    buffer[63] = bitlen;
    buffer[62] = bitlen >> 8;
    buffer[61] = bitlen >> 16;
    buffer[60] = bitlen >> 24;
    buffer[59] = bitlen >> 32;
    buffer[58] = bitlen >> 40;
    buffer[57] = bitlen >> 48;
    buffer[56] = bitlen >> 56;
    sha256_transform(state, buffer);

    // Output the result
    for (int i = 0; i < 8; i++) {
        out[i * 4] = (state[i] >> 24) & 0xFF;
        out[i * 4 + 1] = (state[i] >> 16) & 0xFF;
        out[i * 4 + 2] = (state[i] >> 8) & 0xFF;
        out[i * 4 + 3] = state[i] & 0xFF;
    }
}

int main() {
    // Test SHA-256
    const char *input = "Hello, Blockchain!";
    uint8_t hash[32];
    sha256_hash((const uint8_t *)input, strlen(input), hash);

    printf("SHA-256 hash: ");
    for (int i = 0; i < 32; i++) {
        printf("%02x", hash[i]);
    }
    printf("\n");

    return 0;
}


